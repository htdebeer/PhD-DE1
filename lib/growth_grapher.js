(function() {
  ...
  CoffeeGrounds.Line = (function() {
    ...
    _Class.prototype.add_freehand_line = function(p, path) {
      ...
      if (this.record) {
        time = Date.now() - this.start_time;
        return this.history.push("AF" + p + "@" + time);
      }
    };
  ...
  window.Graph = (function() {
    ...
    function _Class(paper, x, y, width, height, properties) {
      ...
      this.computer_line = new CoffeeGrounds.Line(this.ORIGIN.x, this.ORIGIN.y - this.GRAPH_HEIGHT, this.GRAPH_WIDTH, this.GRAPH_HEIGHT, this.x_axis.unit, this.y_axis.unit, false);
      this.delta_y = 50;
    ...
    _Class.prototype.delta_move = function(e, x, y) {
      var EPSILON, bigstep, dx, dy, factor, length, mx_length, p, path, patharr, point, pointn, seg;
        ...
        if (axis.orientation === 'vertical') {
          ...
          if (ticks[i].label) {
            if (this.y_axis.unit.symbol === 'cm/ml') {
              labels.push(this.paper.text(origin.x - TICKSLENGTH - LABELSEP * 2, origin.y - d, "" + (label.toFixed(1))));
            } else {
